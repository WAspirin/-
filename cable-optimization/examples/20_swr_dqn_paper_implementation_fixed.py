"""
SW-RDQN 完整实现 - 根据论文《基于关键路径点的深度强化学习复合路径规划》

修复问题:
1. 网络输入维度修复 (3D: [B, T, F])
2. OMP 库冲突处理
3. 状态序列管理
4. 优先经验回放 (论文式 19)

核心创新:
1. 阶段关键点机制 (stage-keys) - 多阶段子目标分解
2. 轨迹预测模块 - RNN 预测动态障碍物轨迹
3. 优先经验回放 - 基于奖励+TD 误差的加权采样 (论文式 19)
4. 网络结构 - CNN + LSTM + Dueling DQN

作者：智子 (Sophon)
日期：2026-03-11
"""

# 解决 OMP 库冲突
import os
os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from collections import deque
import random
import math
from typing import List, Tuple, Dict
import heapq


# ============================================================================
# 1. 经验回放池 - 论文式 19 优先采样
# ============================================================================

class PrioritizedReplayBuffer:
    """基于奖励和 TD 误差的优先经验回放 (论文式 19)"""
    
    def __init__(self, capacity: int = 10000, 
                 w_reward: float = 0.5, 
                 w_td_error: float = 0.5):
        self.capacity = capacity
        self.buffer = deque(maxlen=capacity)
        self.priorities = deque(maxlen=capacity)
        self.w_reward = w_reward
        self.w_td_error = w_td_error
    
    def add(self, transition: Dict, reward: float, td_error: float):
        """
        添加经验并计算优先级 (论文式 19)
        
        P(i) = [w_reward * log(reward_i + |min(reward_i, 0)| + 1) 
              + w_td_error * log(|TD_error_i| + 1)] / sum
        """
        self.buffer.append(transition)
        
        # 计算优先级 (式 19)
        reward_term = self.w_reward * math.log(reward + abs(min(reward, 0)) + 1)
        td_term = self.w_td_error * math.log(abs(td_error) + 1)
        priority = reward_term + td_term
        
        self.priorities.append(priority)
    
    def sample(self, batch_size: int) -> Tuple[List, List, List]:
        """按优先级采样"""
        if len(self.buffer) < batch_size:
            return list(self.buffer), None, None
        
        # 归一化优先级为概率
        priorities = np.array(self.priorities)
        probabilities = priorities / (priorities.sum() + 1e-6)
        
        # 按概率采样
        indices = np.random.choice(len(self.buffer), batch_size, p=probabilities)
        batch = [self.buffer[i] for i in indices]
        
        return batch, indices, probabilities[indices]
    
    def update_priorities(self, indices: List[int], new_priorities: List[float]):
        """更新指定经验的优先级"""
        for idx, priority in zip(indices, new_priorities):
            if 0 <= idx < len(self.priorities):
                self.priorities[idx] = priority
    
    def __len__(self):
        return len(self.buffer)


# ============================================================================
# 2. 网络结构 - 论文图 4: CNN + LSTM + Dueling DQN
# ============================================================================

class SWRDQNNetwork(nn.Module):
    """
    SW-RDQN 网络结构 (论文图 4)
    
    输入：
        grid_input: (batch, time_steps, map_size, map_size) - 4D
        spatial_input: (batch, time_steps, spatial_features) - 3D
    输出：Q 值 (batch, n_actions)
    """
    
    def __init__(self, spatial_features: int = 6, 
                 hidden_size: int = 256,
                 n_actions: int = 4,
                 time_steps: int = 4,
                 map_size: int = 24):
        super(SWRDQNNetwork, self).__init__()
        
        self.time_steps = time_steps
        self.map_size = map_size
        self.spatial_features = spatial_features
        
        # 空间关系编码 (论文 3.2 节)
        self.spatial_encoder = nn.Sequential(
            nn.Linear(spatial_features, 64),
            nn.ReLU()
        )
        
        # 场景特征提取 - 双层 CNN
        self.cnn = nn.Sequential(
            # 第一层：5x5 卷积
            nn.Conv2d(1, 32, kernel_size=5, stride=1, padding=2),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            # 第二层：3x3 卷积，步长 2 下采样
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.Flatten()
        )
        
        # 计算 CNN 输出维度
        cnn_output_size = 64 * (map_size // 2) * (map_size // 2)
        
        # LSTM 层 - 时序建模
        self.lstm = nn.LSTM(
            input_size=cnn_output_size + 64,  # CNN 输出 + 空间关系特征
            hidden_size=hidden_size,
            num_layers=2,
            batch_first=True,
            dropout=0.3
        )
        
        # Dueling DQN 分支
        self.value_branch = nn.Sequential(
            nn.Linear(hidden_size, hidden_size // 2),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(hidden_size // 2, 1)
        )
        
        self.advantage_branch = nn.Sequential(
            nn.Linear(hidden_size, hidden_size // 2),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(hidden_size // 2, n_actions)
        )
    
    def forward(self, grid_input: torch.Tensor, 
                spatial_input: torch.Tensor,
                hidden_state: tuple = None):
        """
        前向传播
        
        Args:
            grid_input: 栅格图输入 (batch, time_steps, map_size, map_size)
            spatial_input: 空间关系特征 (batch, time_steps, spatial_features)
            hidden_state: LSTM 隐藏状态
        
        Returns:
            q_values: Q 值 (batch, n_actions)
            hidden_state: 新的 LSTM 隐藏状态
        """
        batch_size = grid_input.shape[0]
        
        # CNN 特征提取
        # 重塑为 (batch*time_steps, 1, map_size, map_size)
        grid_flat = grid_input.reshape(-1, 1, self.map_size, self.map_size)
        cnn_feat = self.cnn(grid_flat)
        # 重塑回 (batch, time_steps, cnn_features)
        cnn_feat = cnn_feat.view(batch_size, self.time_steps, -1)
        
        # 空间关系编码
        # spatial_input: (batch, time_steps, spatial_features)
        spatial_feat = self.spatial_encoder(spatial_input)
        
        # 特征融合
        fused = torch.cat([cnn_feat, spatial_feat], dim=2)
        
        # LSTM 时序建模
        lstm_out, hidden_state = self.lstm(fused, hidden_state)
        
        # 取最后时间步
        last_hidden = lstm_out[:, -1, :]
        
        # Dueling DQN 分支
        value = self.value_branch(last_hidden)
        advantage = self.advantage_branch(last_hidden)
        
        # 组合 (论文式 17)
        q_values = value + (advantage - advantage.mean(dim=1, keepdim=True))
        
        return q_values, hidden_state


# ============================================================================
# 3. 奖励函数 - 论文式 12-16
# ============================================================================

class RewardFunction:
    """
    复合奖励函数 (论文式 16)
    
    Rround = Rgoal + Ralign + Ravoid + Rpre + Rstep
    """
    
    def __init__(self, config: Dict = None):
        # 奖励参数 (论文表 2)
        self.config = config or {
            'r_goal': 100.0,      # 到达目标奖励
            'r_align': 1.0,       # 对齐奖励系数
            'r_avoid_1': -3.0,    # 避障奖励 (d < 1.0)
            'r_avoid_2': -2.0,    # 避障奖励 (1.0 < d < 2.0)
            'r_avoid_3': -1.0,    # 避障奖励 (2.0 < d < 3.0)
            'r_pre': -1.25,       # 预测落点惩罚系数
            'r_step': -0.01,      # 步数惩罚
            'dis_threshold': 2.0  # 预测距离阈值
        }
    
    def calculate_reward(self, state: Dict, action: int, 
                        next_state: Dict, done: bool) -> float:
        """计算总奖励 (论文式 16)"""
        r_goal = self._r_goal(done, next_state)
        r_align = self._r_align(state, action, next_state)
        r_avoid = self._r_avoid(next_state)
        r_pre = self._r_pre(next_state)
        r_step = self.config['r_step']
        
        total_reward = r_goal + r_align + r_avoid + r_pre + r_step
        
        return total_reward
    
    def _r_goal(self, done: bool, next_state: Dict) -> float:
        """到达目标奖励 (论文式 11)"""
        if done:
            return self.config['r_goal']
        return 0.0
    
    def _r_align(self, state: Dict, action: int, next_state: Dict) -> float:
        """轨迹对齐奖励 (论文式 12)"""
        robot_pos = np.array(next_state['position'])
        key_point = np.array(next_state['current_key_point'])
        
        expected_dir = key_point - robot_pos
        if np.linalg.norm(expected_dir) < 1e-6:
            return 0.0
        
        expected_dir = expected_dir / np.linalg.norm(expected_dir)
        
        actual_dir = robot_pos - np.array(state['position'])
        if np.linalg.norm(actual_dir) < 1e-6:
            return 0.0
        
        actual_dir = actual_dir / np.linalg.norm(actual_dir)
        
        cos_theta = np.dot(expected_dir, actual_dir)
        
        return self.config['r_align'] * cos_theta
    
    def _r_avoid(self, next_state: Dict) -> float:
        """避障奖励 (论文式 13-14)"""
        d_obstacle = next_state['distance_to_obstacle']
        
        if d_obstacle < 1.0:
            return self.config['r_avoid_1']
        elif d_obstacle < 2.0:
            return self.config['r_avoid_2']
        elif d_obstacle < 3.0:
            return self.config['r_avoid_3']
        else:
            return 0.0
    
    def _r_pre(self, next_state: Dict) -> float:
        """预测落点靠近惩罚 (论文式 15)"""
        dis = next_state['distance_to_predicted_obstacle']
        
        if dis < self.config['dis_threshold']:
            return self.config['r_pre'] * dis
        else:
            return 0.0


# ============================================================================
# 4. SW-RDQN 智能体 - 完整实现
# ============================================================================

class SWRDQNAgent:
    """
    SW-RDQN 智能体
    
    核心特性:
    1. 阶段关键点机制
    2. 优先经验回放 (论文式 19)
    3. Dueling DQN 网络
    """
    
    def __init__(self, spatial_features: int = 6, 
                 n_actions: int = 4,
                 config: Dict = None):
        
        self.config = config or {
            'gamma': 0.99,
            'lr': 1e-4,
            'batch_size': 64,
            'buffer_size': 10000,
            'epsilon_start': 1.0,
            'epsilon_end': 0.01,
            'epsilon_decay': 0.995,
            'target_update_freq': 100,
            'time_steps': 4,
            'map_size': 24
        }
        
        self.n_actions = n_actions
        self.epsilon = self.config['epsilon_start']
        self.steps = 0
        
        # 网络
        self.network = SWRDQNNetwork(
            spatial_features=spatial_features,
            n_actions=n_actions,
            time_steps=self.config['time_steps'],
            map_size=self.config['map_size']
        )
        self.target_network = SWRDQNNetwork(
            spatial_features=spatial_features,
            n_actions=n_actions,
            time_steps=self.config['time_steps'],
            map_size=self.config['map_size']
        )
        self.target_network.load_state_dict(self.network.state_dict())
        self.target_network.eval()
        
        # 优化器
        self.optimizer = torch.optim.Adam(
            self.network.parameters(), 
            lr=self.config['lr']
        )
        
        # 经验回放池 (论文式 19)
        self.replay_buffer = PrioritizedReplayBuffer(
            capacity=self.config['buffer_size']
        )
        
        # 奖励函数
        self.reward_function = RewardFunction()
        
        # 阶段关键点
        self.key_points = []
        self.current_key_idx = 0
        
        # 经验序列 (time_steps 长度)
        self.grid_sequence = deque(maxlen=self.config['time_steps'])
        self.spatial_sequence = deque(maxlen=self.config['time_steps'])
    
    def _create_grid_map(self, state: Dict) -> np.ndarray:
        """创建栅格地图"""
        grid = np.zeros((self.config['map_size'], self.config['map_size']))
        
        # 填充障碍物
        obstacles = state.get('obstacles', [])
        for obs in obstacles:
            x, y = int(obs[0]), int(obs[1])
            if 0 <= x < self.config['map_size'] and 0 <= y < self.config['map_size']:
                grid[x, y] = 1.0
        
        # 标记机器人位置
        pos = state.get('position', [0, 0])
        if 0 <= int(pos[0]) < self.config['map_size'] and 0 <= int(pos[1]) < self.config['map_size']:
            grid[int(pos[0]), int(pos[1])] = 0.5
        
        return grid
    
    def _create_spatial_features(self, state: Dict) -> np.ndarray:
        """创建空间关系特征"""
        features = np.zeros(6)
        
        pos = state.get('position', [0, 0])
        key_point = state.get('current_key_point', [0, 0])
        
        features[0] = pos[0] / self.config['map_size']  # 归一化
        features[1] = pos[1] / self.config['map_size']
        features[2] = key_point[0] / self.config['map_size']
        features[3] = key_point[1] / self.config['map_size']
        features[4] = state.get('distance_to_obstacle', 0) / 10.0
        features[5] = state.get('distance_to_predicted_obstacle', 0) / 10.0
        
        return features
    
    def select_action(self, state: Dict, train: bool = True) -> int:
        """选择动作 (ε-greedy)"""
        if train and random.random() < self.epsilon:
            return random.randint(0, self.n_actions - 1)
        
        # 准备输入
        grid, spatial = self._prepare_state_input(state)
        
        with torch.no_grad():
            q_values, _ = self.network(
                grid.unsqueeze(0),  # (1, time_steps, map_size, map_size)
                spatial.unsqueeze(0)  # (1, time_steps, spatial_features)
            )
            return q_values.argmax(dim=1).item()
    
    def _prepare_state_input(self, state: Dict):
        """
        准备状态输入 (3D 输入)
        
        Returns:
            grid_input: (time_steps, map_size, map_size)
            spatial_input: (time_steps, spatial_features)
        """
        # 创建当前帧
        grid_frame = self._create_grid_map(state)
        spatial_frame = self._create_spatial_features(state)
        
        # 添加到序列
        self.grid_sequence.append(grid_frame)
        self.spatial_sequence.append(spatial_frame)
        
        # 如果序列不够长，用当前帧填充
        while len(self.grid_sequence) < self.config['time_steps']:
            self.grid_sequence.append(grid_frame)
            self.spatial_sequence.append(spatial_frame)
        
        # 转换为张量
        grid_input = torch.FloatTensor(np.array(self.grid_sequence))
        spatial_input = torch.FloatTensor(np.array(self.spatial_sequence))
        
        return grid_input, spatial_input
    
    def store_transition(self, state: Dict, action: int, 
                        reward: float, next_state: Dict, done: bool,
                        td_error: float = None):
        """存储转移 (论文式 19)"""
        transition = {
            'state': state,
            'action': action,
            'reward': reward,
            'next_state': next_state,
            'done': done
        }
        
        if td_error is None:
            td_error = abs(reward)
        
        self.replay_buffer.add(transition, reward, td_error)
    
    def update(self) -> float:
        """更新网络"""
        if len(self.replay_buffer) < self.config['batch_size']:
            return 0.0
        
        # 优先采样
        batch, indices, probabilities = self.replay_buffer.sample(
            self.config['batch_size']
        )
        
        # 准备批量数据
        grids = []
        spatials = []
        actions = []
        rewards = []
        next_grids = []
        next_spatials = []
        dones = []
        
        for t in batch:
            # 为每个转移准备序列输入
            grid, spatial = self._prepare_state_input_for_batch(t['state'])
            next_grid, next_spatial = self._prepare_state_input_for_batch(t['next_state'])
            
            grids.append(grid)
            spatials.append(spatial)
            actions.append(t['action'])
            rewards.append(t['reward'])
            next_grids.append(next_grid)
            next_spatials.append(next_spatial)
            dones.append(t['done'])
        
        # 转换为张量 (batch, time_steps, ...)
        grids = torch.stack(grids)
        spatials = torch.stack(spatials)
        actions = torch.LongTensor(actions)
        rewards = torch.FloatTensor(rewards)
        next_grids = torch.stack(next_grids)
        next_spatials = torch.stack(next_spatials)
        dones = torch.FloatTensor(dones)
        
        # 计算当前 Q 值
        q_values, _ = self.network(grids, spatials)
        q_values = q_values.gather(1, actions.unsqueeze(1)).squeeze(1)
        
        # 计算目标 Q 值
        with torch.no_grad():
            next_q_values, _ = self.target_network(next_grids, next_spatials)
            next_q_max = next_q_values.max(dim=1)[0]
            targets = rewards + (self.config['gamma'] ** self.config['time_steps']) * next_q_max * (1 - dones)
        
        # 计算损失
        loss = F.smooth_l1_loss(q_values, targets)
        
        # 优化
        self.optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.network.parameters(), max_norm=1.0)
        self.optimizer.step()
        
        # 更新优先级
        if indices is not None:
            new_priorities = []
            for i, t in zip(indices, batch):
                with torch.no_grad():
                    grid, spatial = self._prepare_state_input_for_batch(t['state'])
                    next_grid, next_spatial = self._prepare_state_input_for_batch(t['next_state'])
                    
                    q = self.network(grid.unsqueeze(0), spatial.unsqueeze(0)).gather(
                        1, torch.LongTensor([t['action']])
                    ).item()
                    next_q = self.target_network(next_grid.unsqueeze(0), next_spatial.unsqueeze(0)).max().item()
                    
                    td_error = abs(t['reward'] + self.config['gamma'] * next_q * (1 - t['done']) - q)
                    new_priorities.append(td_error)
            
            self.replay_buffer.update_priorities(indices, new_priorities)
        
        # 更新目标网络
        self.steps += 1
        if self.steps % self.config['target_update_freq'] == 0:
            self.target_network.load_state_dict(self.network.state_dict())
        
        # 衰减 ε
        self.epsilon = max(
            self.config['epsilon_end'],
            self.epsilon * self.config['epsilon_decay']
        )
        
        return loss.item()
    
    def _prepare_state_input_for_batch(self, state: Dict):
        """为批量处理准备状态输入"""
        # 创建临时序列
        temp_grid_seq = deque(maxlen=self.config['time_steps'])
        temp_spatial_seq = deque(maxlen=self.config['time_steps'])
        
        grid_frame = self._create_grid_map(state)
        spatial_frame = self._create_spatial_features(state)
        
        # 用当前帧填充
        for _ in range(self.config['time_steps']):
            temp_grid_seq.append(grid_frame)
            temp_spatial_seq.append(spatial_frame)
        
        grid_input = torch.FloatTensor(np.array(temp_grid_seq))
        spatial_input = torch.FloatTensor(np.array(temp_spatial_seq))
        
        return grid_input, spatial_input
    
    def set_key_points(self, key_points: List[Tuple[int, int]]):
        """设置阶段关键点"""
        self.key_points = key_points
        self.current_key_idx = 0
    
    def get_current_key_point(self) -> Tuple[int, int]:
        """获取当前阶段关键点"""
        if self.current_key_idx < len(self.key_points):
            return self.key_points[self.current_key_idx]
        return None
    
    def update_key_point(self, robot_pos: Tuple[int, int], threshold: float = 3.0):
        """更新关键点索引"""
        if self.current_key_idx < len(self.key_points):
            key_point = self.key_points[self.current_key_idx]
            distance = np.linalg.norm(np.array(robot_pos) - np.array(key_point))
            
            if distance < threshold:
                self.current_key_idx += 1


# ============================================================================
# 5. 主函数 - 测试
# ============================================================================

def main():
    """测试 SW-RDQN 智能体"""
    print("="*60)
    print("SW-RDQN 完整实现测试（修复版）")
    print("="*60)
    
    # 创建智能体
    agent = SWRDQNAgent(
        spatial_features=6,
        n_actions=4,
        config={
            'gamma': 0.99,
            'lr': 1e-4,
            'batch_size': 64,
            'buffer_size': 10000,
            'epsilon_start': 1.0,
            'epsilon_end': 0.01,
            'epsilon_decay': 0.995,
            'target_update_freq': 100,
            'time_steps': 4,
            'map_size': 24
        }
    )
    
    print(f"✓ 智能体创建成功")
    print(f"  网络参数量：{sum(p.numel() for p in agent.network.parameters()):,}")
    print(f"  动作空间：{agent.n_actions}")
    print(f"  经验回放池容量：{agent.config['buffer_size']}")
    print(f"  时间步长：{agent.config['time_steps']}")
    print(f"  地图大小：{agent.config['map_size']}x{agent.config['map_size']}")
    
    # 模拟训练
    print("\n开始模拟训练...")
    n_episodes = 10
    
    for episode in range(n_episodes):
        state = {
            'position': [0, 0],
            'current_key_point': [10, 10],
            'distance_to_obstacle': 5.0,
            'distance_to_predicted_obstacle': 10.0,
            'obstacles': []
        }
        
        total_reward = 0
        done = False
        steps = 0
        
        while not done and steps < 100:
            action = agent.select_action(state, train=True)
            
            next_state = state.copy()
            reward = agent.reward_function.calculate_reward(state, action, next_state, False)
            
            agent.store_transition(state, action, reward, next_state, False)
            
            if len(agent.replay_buffer) >= agent.config['batch_size']:
                loss = agent.update()
            
            state = next_state
            total_reward += reward
            steps += 1
        
        print(f"  回合 {episode+1}/{n_episodes}: "
              f"奖励={total_reward:.2f}, "
              f"步数={steps}, "
              f"ε={agent.epsilon:.3f}")
    
    print("\n✓ 测试完成!")
    print("="*60)


if __name__ == "__main__":
    main()
