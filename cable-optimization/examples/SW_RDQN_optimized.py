...
class SWRDQNNetwork(nn.Module):
    ...
    def forward(self, grid_input: torch.Tensor, 
        ...
        Args:
            ...
            spatial_input: 空间关系特征 (batch, input_features)
        ...
        grid_flat = grid_input.view(-1, 1, self.map_size, self.map_size)
        ...
        # 重复以匹配时间维度
        spatial_feat = spatial_feat.unsqueeze(1).repeat(1, self.time_steps, 1)
...
class SWRDQNAgent:
    ...
    def select_action(self, state: Dict, train: bool = True) -> int:
        ...
        # 准备输入
        grid_input, spatial_input = self._prepare_state_input(state)
        
        with torch.no_grad():
            q_values, _ = self.network(
                grid_input.unsqueeze(0), 
                spatial_input.unsqueeze(0)
            )
    ...
    def _prepare_state_input(self, state: Dict):
        ...
        return torch.FloatTensor(grid), torch.FloatTensor(features)
    ...
    def update(self) -> float:
        ...
        for t in batch:
            grid, spatial = self._prepare_state_input(t['state'])
            next_grid, next_spatial = self._prepare_state_input(t['next_state'])
