"""
SW-RDQN 复合路径规划系统 - 完整复现 Sun et al. (2026)

论文：Sun, X., et al. (2026). Deep reinforcement learning-based composite path 
planning with key path points. IEEE Transactions on Robotics.

架构:
1. Voronoi 图关键点生成 (Algorithm 1)
2. SW-RDQN 局部控制器 (CNN + GRU + Dueling DQN)
3. 加权优先经验回放 (式 17-19)
4. 多阶段 MDP 建模
5. A* 底层规划器
...
# ============================================================================
# Part 1: Voronoi 关键点生成 (Algorithm 1)
# ============================================================================

class VoronoiKeyPointGenerator:
    """Voronoi 关键点生成器 - 复现论文 Algorithm 1"""
    ...
    def extract_key_points(self, start: Tuple[int, int], goal: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        vor = Voronoi(self.obstacles.astype(float))
        skeleton_edges = []
    ...
    def visualize(self, save_path: str = None):
        ...
        ax.legend()
        ax.set_title('Voronoi Key Points')
        if save_path:
...
class SWRDQNNetwork(nn.Module):
    ...
    def __init__(self, obs_height: int = 11, obs_width: int = 11, 
                 n_channels: int = 5, window_size: int = 4,
...
class WeightedPrioritizedReplayBuffer:
    ...
    def add(self, transition: Dict, priority: float = 1.0):
...
# ============================================================================
# Part 5: 局部 SW-RDQN 控制器
# ============================================================================

class LocalSWRDQNController:
    """局部 SW-RDQN 控制器"""
    def __init__(self, obs_size: int = 11, n_channels: int = 5, window_size: int = 4, n_actions: int = 8, learning_rate: float = 1e-4, gamma: float = 0.99):
        ...
        for i in range(self.obs_size):
            for j in range(self.obs_size):
                ...
                if 0 <= gx < grid_map.shape[0] and 0 <= gy < grid_map.shape[1]:
                    ...
                    obs[0, i, j] = 1.0
        goal_dir_x = (goal[0] - x) / max(1, abs(goal[0] - x) + abs(goal[1] - y))
        goal_dir_y = (goal[1] - y) / max(1, abs(goal[0] - x) + abs(goal[1] - y))
        obs[4, :, :] = np.clip((goal_dir_x + goal_dir_y) / 2, -1, 1)
        return obs
    ...
    def store_transition(self, state, action, reward, next_state, done, priority: float = 1.0):
        self.replay_buffer.add({'state': state, 'action': action, 'reward': reward, 'next_state': next_state, 'done': done}, priority)
    ...
    def train_episode(self, env, start: Tuple[int, int], goal: Tuple[int, int], dynamic_obstacles: List[DynamicObstacle], max_steps: int = 200) -> Tuple[float, bool]:
        ...
        for step in range(max_steps):
            ...
            if len(self.obs_window) >= self.window_size:
                ...
                self.store_transition(obs_seq, action, reward, next_obs_seq, done, td_error)
            if step % 4 == 0:
...
def main():
    ...
    print("2. 创建 SW-RDQN 规划器...")
    planner = SWRDQNPlanner(grid_map, safety_distance=2.0)
    print(f"   ✓ Voronoi 关键点生成器\n   ✓ SW-RDQN 控制器 (CNN+GRU+Dueling)\n   ✓ 加权优先经验回放\n   ✓ A* 底层规划器\n")
    print("3. 规划路径...")
    path, stats = planner.plan((2, 2), (27, 27), dynamic_obstacles, use_astar=True, train_first=True)
    print("4. 可视化...")
    if path:
        planner.visualize(path, save_path='/root/.openclaw/workspace/cable-optimization/examples/outputs/swrdqn_path.png')
    print("\n" + "="*60 + "\n✅ SW-RDQN 完整复现完成！\n" + "="*60)
    print("\n核心组件:")
    print("  1. ✅ Voronoi 关键点生成 (Algorithm 1)")
    print("  2. ✅ SW-RDQN 网络 (CNN+GRU+Dueling DQN)")
    print("  3. ✅ 加权优先经验回放 (式 17-19)")
    print("  4. ✅ 多阶段 MDP 建模")
    print("  5. ✅ A* 底层规划器")
    print("  6. ✅ 动态障碍物处理")
    print("\n输出：swrdqn_path.png\n")
