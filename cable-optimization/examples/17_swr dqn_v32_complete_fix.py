"""
SW-RDQN v3.2 - 全面修复所有问题

修复内容 (v3.1 → v3.2):
1. ✅ 目标方向编码改为 7 通道 (dx + dy 分离)
2. ✅ _calculate_safety_distance 使用全局地图
3. ✅ update_priorities zip 解包修复
4. ✅ DynamicObstacle 类型注解修正
5. ✅ 添加梯度裁剪
6. ✅ 其他小问题修复
...
class VoronoiKeyPointGenerator:
    ...
    def visualize(self, save_path: str = None):
        ...
        ax.legend()
        ax.set_title('Voronoi Key Points (v3.2)')
        if save_path:
...
class DynamicObstacle:
    ...
    def __init__(self, x: float, y: float, vx: float, vy: float, size: int = 2, prediction_horizon: int = 5):
        self.x, self.y = float(x), float(y)  # 修复：改为 float
...
class SWRDQNNetwork(nn.Module):
    ...
    def __init__(self, obs_height: int = 11, obs_width: int = 11, 
                 n_channels: int = 7, window_size: int = 4,  # 修复：7 通道
...
# ============================================================================
# Part 5: 局部 SW-RDQN 控制器 (全面修复)
# ============================================================================

class LocalSWRDQNController:
    """局部 SW-RDQN 控制器 - 全面修复版"""
    def __init__(self, obs_size: int = 11, n_channels: int = 7, window_size: int = 4, 
                 n_actions: int = 8, learning_rate: float = 1e-4, gamma: float = 0.99,
                 grid_map: np.ndarray = None):  # 修复：添加全局地图
        self.obs_size = obs_size
        self.n_channels = n_channels  # 7 通道
        self.window_size = window_size
        self.n_actions = n_actions
        self.gamma = gamma
        self.grid_map = grid_map  # 修复：保存全局地图
    ...
    def get_observation(self, grid_map: np.ndarray, position: Tuple[int, int], goal: Tuple[int, int], dynamic_obstacles: List[DynamicObstacle]) -> np.ndarray:
        ...
        obs[6, :, :] = np.clip(goal_dy, -1, 1)  # 通道 6: dy (修复!)
...
# ============================================================================
# Part 7: 完整 SW-RDQN 规划系统 v3.2
# ============================================================================

class SWRDQNPlanner:
    """SW-RDQN 复合路径规划系统 v3.2"""
    def __init__(self, grid_map: np.ndarray, safety_distance: float = 2.0):
        self.grid_map = grid_map
        self.env = GridEnvironment(grid_map)
        self.keypoint_generator = VoronoiKeyPointGenerator(grid_map, safety_distance)
        self.local_controller = LocalSWRDQNController(grid_map=grid_map)  # 修复：传入全局地图
...
def main():
    ...
    print("2. 创建 SW-RDQN 规划器 v3.2...")
    planner = SWRDQNPlanner(grid_map, safety_distance=2.0)
    print(f"   ✓ Voronoi 关键点生成器 (障碍物中心)\n   ✓ SW-RDQN 控制器 (7 通道 + 梯度裁剪)\n   ✓ 加权优先经验回放 (式 17)\n   ✓ A* 底层规划器\n")
    print("3. 规划路径...")
    path, stats = planner.plan((2, 2), (27, 27), dynamic_obstacles, use_astar=True, train_first=True)
    print("4. 可视化...")
    if path:
        planner.visualize(path, save_path='/root/.openclaw/workspace/cable-optimization/examples/outputs/swrdqn_v32_path.png')
    print("\n" + "="*60 + "\n✅ SW-RDQN v3.2 全面修复完成！\n" + "="*60)
    print("\n修复内容 (v3.1 → v3.2):")
    print("  1. ✅ 目标方向编码改为 7 通道 (dx + dy 分离)")
    print("  2. ✅ _calculate_safety_distance 使用全局地图")
    print("  3. ✅ update_priorities zip 解包修复")
    print("  4. ✅ DynamicObstacle 类型注解修正")
    print("  5. ✅ 添加梯度裁剪 (max_norm=1.0)")
    print("\n输出：swrdqn_v32_path.png\n")
